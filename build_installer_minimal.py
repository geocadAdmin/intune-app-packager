"""
Build standalone installer with embedded Python - MINIMAL VERSION
This creates a single executable for GUI only (no Graph API dependencies).
Use this on Windows ARM64 where aiohttp/httptools don't have pre-built wheels.
"""
...
def main():
    print_step("Building Standalone Installer (Minimal)")
    ...
    if not run_command([sys.executable, "-m", "pip", "install", "-r", "requirements-minimal.txt"], 
                      "Install minimal dependencies"):
        sys.exit(1)
...
a = Analysis(
    ...
    hooksconfig={{}},
    runtime_hooks=[],
    excludes=['aiohttp', 'fastapi', 'uvicorn', 'msal', 'httptools'],
...
"""
    ...
    spec_file = project_dir / "installer_minimal.spec"
    ...
    if exe_path.exists():
        ...
        print(f"\n   To run: .\\{exe_name}")
        print(f"\n⚠️  NOTE: This minimal build includes:")
        print(f"   ✅ PowerShell script generator (GUI)")
        print(f"   ✅ All script templates")
        print(f"   ❌ Microsoft Graph API (requires full install)")
    else:
